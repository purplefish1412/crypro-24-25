    ...
    if (os.path.isfile(InputFileName)):
        with open(InputFileName, "r", encoding="utf-8") as InputFile:
            while True:
                ...
                if(Sum % 10 == 0):
                    print("Processing char № "+str(Sum), end='\r')
                    
                if not char:
                    print("\n")
                    print("Processing of single characters completed!", end='\r')
                    break
...
def CalculateBigramFrequency(InputFileName, CharArray, DoublePass = True):    
    ...
    for i in range(0, len(CharArray)):
        for j in range(0, len(CharArray)):
            ResultDict.update({CharArray[i]+CharArray[j]: [0,0]})
        ...
        with open(InputFileName, "r", encoding="utf-8") as InputFile:
            for i in range(0,2):
                ...
                while True:
                    ...
                    if(Sum % 100 == 0):
                        print("Processing char № "+str(Sum))
                        
                    if not char:
                        print("Processing of bigrams completed!")
                        break

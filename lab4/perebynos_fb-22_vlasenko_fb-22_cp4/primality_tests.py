from helpers import gcd_extended_euclid, horner_pow, jacobi_symbol
from random import randint

# gcd(a, p) = 1 and a^(p-1) = 1 mod p
def check_pseudo_prime(base: int, p: int) -> bool:
    if gcd_extended_euclid(base, p)[0] != 1:
        return False
    
    return horner_pow(base, p-1, p) == 1
...
def check_pseudo_prime_jacobi_euler(base: int, p: int) -> bool:
    ...
    return (jacobi_symbol(base, p) % p) == horner_pow(base, (p-1)//2, p)
...
def _check_pseudo_prime_strong(base: int, p: int, s: int, d: int) -> bool:
    if horner_pow(base, d, p) == 1:
        return True

    for r in range(s):
        if horner_pow(base, d * (1 << r), p) == (-1) % p:
            return True
...
def trial_division_test(p: int, probes: int) -> bool:
    ...
    probes = min(p//2, probes)
    if p % 2 == 0:
        return False
    
    for i in range(3, probes+1, 2):
        if p % i == 0:
            return False
